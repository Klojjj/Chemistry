...
n = int(input())
print('PLEASE, ENTER REACTANT CONSTANTS WITH OPPOSITE SIGN!!!')
a_r, b_r, c_r = chem.calculate_capacity(n)
print('Choose one option:', '1 - calculate entropy', '2 - calculate enthalpy', "3 - calculate Gibb's energy", '4 - all')
flag = int(input())
if flag in (1, 3, 4):
    entropy = chem.calculate_entropy(a_r, b_r, c_r, t_1, t_2, n)
    if flag in (1, 4):
        print("Entropy = ", entropy * 10 ** 4 // 10 / 10 ** 3, "J")
if flag in (2, 3, 4):
    enthalpy = chem.calculate_enthalpy(a_r, b_r, c_r, t_1, t_2, n)
    if flag in (2, 4):
